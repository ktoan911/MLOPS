...
# BeautifulSoup là một công cụ để phân tích HTML/XML một cách đơn giản, hiệu quả. 
# Khi bạn đã có nội dung HTML, nó cho phép bạn tìm, duyệt và trích xuất thông tin từ thẻ HTML dễ dàng.
from bs4 import BeautifulSoup
from loguru import logger

# Selenium là một công cụ để tự động điều khiển trình duyệt web (Chrome, Firefox,...). 
# Nó giúp bạn tương tác với trang web giống như người dùng thật: mở trang, click, cuộn, điền form, chờ dữ liệu load v.v.
...
WAIT_TIMEOUT = 10  # wait for some seconds

# Allow a pop-up window to be in cognito mode // ẩn danh
...
# kéo đến cái cuối cùng của trang
...
for deal in deals:
    try:
        ...
        product_name = deal.find_all("span", {"itemprop": "name"})[
            0].get_text()
